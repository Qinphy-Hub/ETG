...
// main function
onMounted(async () => {
  let result = await getUserRoutes(projectId) as any
  originCode.value = result.data
})

const saveCode = async () => {
  let result = await saveUserRoutes(projectId, code.value) as any
...
const initProgram = async () => {
  ElMessageBox.confirm('It will delete your data before. Continue?', 'Warning', {
    ...
    let result = await initialUserRoutes(projectId) as any
    originCode.value = result.data
    ElMessage({
      type: 'success',
