...
import {getCoveredDemandsCode, initialCoveredDemandsCode, saveCoveredDemandsCode} from "@/api/program";
...
// onmount
onMounted(async () => {
  let result = await getCoveredDemandsCode(projectId) as any
...
const saveCode = async () => {
  let result = await saveCoveredDemandsCode(projectId, code.value) as any
...
const initProgram = async () => {
  ElMessageBox.confirm('It will delete your data before. Continue?', 'Warning', {
    ...
    let result = await initialCoveredDemandsCode(projectId) as any
    originCode.value = result.data
    ElMessage({
      type: 'success',
