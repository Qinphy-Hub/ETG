...
// main function
onMounted(async () => {
  let result = await getRouteCode(projectId, programId) as any
  originCode.value = result.data
})

const saveCode = async () => {
  let result = await saveRouteCode(projectId, programId, code.value) as any
...
const initProgram = async () => {
  ElMessageBox.confirm('It will delete your data before. Continue?', 'Warning', {
    ...
    let result = await initialRouteCode(projectId, programId) as any
    originCode.value = result.data
    ElMessage({
      type: 'success',
