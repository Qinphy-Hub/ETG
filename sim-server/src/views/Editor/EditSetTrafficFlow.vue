...
import {
  getSetTrafficFlowCode, initialSetTrafficFlowCode, saveSetTrafficFlowCode
...
// main function
onMounted(async () => {
  let result = await getSetTrafficFlowCode(projectId) as any
  originCode.value = result.data
})

const saveCode = async () => {
  let result = await saveSetTrafficFlowCode(projectId, code.value) as any
...
const initProgram = async () => {
  ElMessageBox.confirm('It will delete your data before. Continue?', 'Warning', {
    ...
    let result = await initialSetTrafficFlowCode(projectId) as any
    originCode.value = result.data
    ElMessage({
      type: 'success',
