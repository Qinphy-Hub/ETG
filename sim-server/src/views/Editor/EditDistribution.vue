...
import {
  getDistribution, initialDistribution, saveDistribution
} from "@/api/program";
...
// main function
onMounted(async () => {
  let result = await getDistribution(projectId) as any
  originCode.value = result.data
})

const saveCode = async () => {
  let result = await saveDistribution(projectId, code.value) as any
...
const initProgram = async () => {
  ElMessageBox.confirm('It will delete your data before. Continue?', 'Warning', {
    ...
    let result = await initialDistribution(projectId) as any
    originCode.value = result.data
    ElMessage({
      type: 'success',
