...
import {getFlpCode, initialFlpCode, saveFlpCode} from "@/api/program";
...
// onmount
onMounted(async () => {
  let result = await getFlpCode(projectId, programId) as any
...
const saveCode = async () => {
  let result = await saveFlpCode(projectId, programId, code.value) as any
...
const initProgram = async () => {
  ElMessageBox.confirm('It will delete your data before. Continue?', 'Warning', {
    ...
    let result = await initialFlpCode(projectId, programId) as any
    originCode.value = result.data
    ElMessage({
      type: 'success',
