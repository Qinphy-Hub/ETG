...
import {Edit, Delete, Failed, Plus, Select, Upload} from "@element-plus/icons-vue";
import {onMounted, ref} from "vue";
import {ElMessage, ElMessageBox, type UploadRequestOptions} from "element-plus";
import {
  deleteRealDemandsData,
  getNodesData, getRealDemandsData, saveRealDemandsData,
  uploadDataService
} from "@/api/load";
...
const setNodes = (nodes: any) => {
  for (const key in nodes) {
    const node = L.marker(nodes[key].point).bindPopup("id: " + key)
...
const setOneStation = (key: string) => {
  const station = L.circleMarker(stations[key].point, {radius: 15, color: "#FFEE00"}).on("click", () => {
    if (propertyFlag) {
      properties.value["key"] = key
      if (stations[key].type === undefined) {
...
const updateData = async () => {
  let stationsRet = await getRealDemandsData(projectId) as any
  let nodesRet = await getNodesData(projectId) as any
  if (nodesRet.data.nodes !== null) nodes = nodesRet.data.nodes
  if (stationsRet.data.realDemands !== null) stations = stationsRet.data.realDemands
  setNodes(nodes)
...
const uploadData = async (param: UploadRequestOptions) => {
  await uploadDataService(projectId, "real-demands", param.file)
  ElMessage.success({
...
const saveData = () => {
  const data = {
    "projectId": projectId,
    "demands": JSON.stringify(stations),
  }
  saveRealDemandsData(data)
  ElMessage.success({
...
const deleteData = () => {
  ElMessageBox.confirm('It will delete stations data file from database. Continue?', 'Warning', {
    ...
    deleteRealDemandsData(projectId)
    clearData()
    ElMessage({
      type: 'success',
...
const addItem = () => {
  if (properties.value.type === "details") {
...
<template>
  ...
  <el-dialog v-model="propertiesVisible" title="Settings" width="500">
    <el-select v-model="properties.type" size="large" style="width: 240px">
      <el-option key="details" label="details" value="details"/>
      <el-option key="distribution" label="distribution" value="distribution"/>
    </el-select>
    <div v-if="properties.type == 'distribution'">
      <el-table :data="properties.data" style="width: 100%" max-height="240">
        <el-table-column prop="param" label="Parameters" align="center">
          <template #default="scope">
            <el-input v-model="properties.data[scope.$index].param" />
          </template>
        </el-table-column>
        <el-table-column prop="value" label="Value" align="center">
          <template #default="scope">
            <el-input-number v-model="properties.data[scope.$index].value" style="width: 100%;" />
          </template>
        </el-table-column>
      </el-table>
    </div>
    <div v-if="properties.type == 'details'">
      <el-table :data="properties.data" style="width: 100%" max-height="240">
        <el-table-column prop="time" label="Time" align="center">
          <template #default="scope">
            <el-input-number v-model="properties.data[scope.$index].time" />
          </template>
        </el-table-column>
        <el-table-column prop="num" label="Num" align="center">
          <template #default="scope">
            <el-input-number v-model="properties.data[scope.$index].num" style="width: 100%;" />
          </template>
        </el-table-column>
      </el-table>
    </div>
