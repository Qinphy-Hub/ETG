...
import {
  deleteODDemandsData, getNodesData, getODPairsData,
  saveODDemandsData,
  uploadDataService
} from "@/api/load";
...
const setOneOD = (key: string) => {
  ...
  const O = L.marker(nodes[odPairs[key].O].point, {icon: startIcon}).on("contextmenu", () => {
    ...
    if (propertyFlag) {
      ...
      properties.value["value"] = odPairs[key].value
  ...
  }).bindPopup("id: " + key + ", value: " + odPairs[key].value)
  odsGroup.addLayer(O)
}
const saveProperties = () => {
  const key = properties.value.key
  odPairs[key].value = properties.value.value
  propertiesVisible.value = false
}

const updateData = async () => {
  clearData()
  let odDemand = await getODPairsData(projectId) as any
...
const uploadData = async (param: UploadRequestOptions) => {
  await uploadDataService(projectId, "od-demands", param.file)
  ElMessage.success({
...
const saveData = async () => {
  ...
  const data = {
    "projectId": projectId,
    "odDemands": JSON.stringify(odPairs)
  }
  await saveODDemandsData(data)
  ElMessage.success({
...
<template>
  <div class="container">
    ...
    <el-tooltip effect="dark" content="show all OD pairs" placement="right">
...
<!-- set demand point properties -->
  <el-dialog v-model="propertiesVisible" title="Shipping address" width="500">
